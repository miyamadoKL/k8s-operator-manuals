#!/usr/bin/env python3
"""Operator ごとの目次 README.md の体裁を検査する。

AGENTS.md §2「目次 README の体裁」で定めた共通の並びに従っているかを静的に確認する。
検査対象は「直下に README.md と part* サブディレクトリを持つディレクトリ」を Operator マニュアル
とみなして自動検出する(入れ子も可。新しい Operator を足しても自動で対象に入る)。
ルート README.md（Operator 一覧）は part* を持たないため対象外。
...
検査項目:
  ...
  5. 導入が箇条書き(対象バージョン / 対象 operator リポジトリ / 想定読者 / 読み方)。
...
def check_readme(path):
    ...
    for label in ("対象バージョン", "対象 operator リポジトリ", "想定読者", "読み方"):
